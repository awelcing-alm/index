...
/* -------------- util fetchers -------------- */
const apiGetNames = async (): Promise<string[]> => (await fetch("/api/templates")).json()
...
const apiDeleteTpl = (name: string) => fetch(`/api/templates/${name}`, { method: "DELETE" })

const emptyTpl = (): Template => ({ name: "", description: "", attributes: {}, createdAt: "" })
...
export default function TemplateBuilderPage() {
  ...
  const saveCurrent = async () => {
    ...
    setTemplates((prev) => {
      const others = prev.filter((t) => t.name !== payload.name)
      return [...others, payload].sort((a, b) => a.name.localeCompare(b.name))
    })
    ...
    <div className="space-y-6">
      <Card className="bg-black/20 backdrop-blur border-white/10">
        <CardHeader>
          ...
          <p className="text-gray-400">Create, save and apply custom templates</p>
        </CardHeader>
        <CardContent>
          <Tabs value={tab} onValueChange={(v) => setTab(v as any)} className="w-full">
            <TabsList className="grid grid-cols-2 border-b border-white/10 bg-transparent">
              <TabsTrigger value="create">{isEditing ? "Edit" : "Create"}</TabsTrigger>
            ...
            <TabsContent value="create" className="mt-6 space-y-6">
              {saved && (
                <Alert className="border-green-500/50 bg-green-500/10 flex items-center gap-2">
                  ...
                  <AlertDescription className="text-green-300">{saved}</AlertDescription>
                </Alert>
              )}
              ...
              <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3 bg-white/5 p-4 border border-white/10 rounded">
                {NEWSLETTER_KEYS.map((slug) => (
                  <div key={slug} className="flex items-center space-x-2">
                    ...
                    />
                    <Label className="text-gray-300 capitalize cursor-pointer">{slug.replace(/-/g, " ")}</Label>
                  </div>
                ))}
              </div>
            ...
            <TabsContent value="manage" className="mt-6 space-y-4">
              ...
              {templates.length === 0 && (
                <Alert>
                  ...
                  <AlertDescription>No custom templates yet. Create one in the Create tab.</AlertDescription>
                </Alert>
              )}
                  ...
                  <CardContent className="p-4 flex items-center justify-between">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className="text-xs text-yellow-300 border-yellow-400/50">Custom</Badge>
                      </div>
                      <p className="text-xs text-gray-400">Created: {new Date(tpl.createdAt).toLocaleDateString()} {tpl.updatedAt && `(Updated ${new Date(tpl.updatedAt).toLocaleDateString()})`}</p>
                      <p className="text-sm text-gray-300 max-w-md truncate" title={tpl.description}>{tpl.description}</p>
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" variant="ghost" onClick={() => { setCurrent(tpl); setIsEditing(true); setTab("create") }}>Edit</Button>
                      <Button size="sm" variant="ghost" onClick={() => { setCurrent({ ...tpl, name: `${tpl.name}-copy`, createdAt: "", updatedAt: undefined }); setIsEditing(false); setTab("create") }}>
                        <Copy className="h-4 w-4" />
                      </Button>
                      <Button size="sm" variant="ghost" className="text-red-400" onClick={() => deleteTpl(tpl.name)}>
