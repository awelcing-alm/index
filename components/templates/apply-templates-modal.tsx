...
export function ApplyTemplatesModal({
  ...
  onApplied,
  initialStack,
}: ApplyTemplatesModalProps): JSX.Element {
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="rounded-none border-line text-ink shadow-sm hover:bg-[hsl(var(--muted))]"
                      onClick={savePreset}
...
export function ApplyTemplatesModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* Full-screen takeover with generous spacing and depth */}
      <DialogContent className="w-[96vw] max-w-none h-[92vh] rounded-none border border-line bg-paper text-ink p-0 shadow-2xl">
        {/* Sticky header for presence and hierarchy */}
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-line bg-paper/95 px-6 py-4 backdrop-blur supports-[backdrop-filter]:bg-paper/80">
          <h2 className="font-serif text-2xl tracking-tight">Apply Templates</h2>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="rounded-none border-line text-ink hover:bg-[hsl(var(--muted))]" onClick={copyShareLink} disabled={selected.length === 0}>Copy Link</Button>
            <Button variant="outline" size="sm" className="rounded-none border-line text-ink hover:bg-[hsl(var(--muted))]" onClick={() => onOpenChange(false)}>Close</Button>
            <Button size="sm" onClick={apply} disabled={selected.length === 0 || loading} className="rounded-none bg-ink px-4 text-paper hover:bg-ink/90">
              <Play className="mr-2 h-4 w-4" /> {dryRun ? "Run Dry Apply" : "Apply"}
            </Button>
          </div>
        </div>
  initialStack?: string[]
        {error && (
          <Alert className="m-6 rounded-none border border-line bg-[hsl(var(--muted))]">
  const [selected, setSelected] = useState<string[]>([])
  const [preview, setPreview] = useState<{ result: Record<string, any>; sources: Record<string, string[]>; conflicts: string[] } | null>(null)
  const [loading, setLoading] = useState(false)

        <div className="grid h-[calc(92vh-84px)] grid-rows-[auto,1fr] gap-0">
          {/* Body grid */}
          <div className="grid flex-1 gap-6 p-6 md:grid-cols-[1.2fr_1fr] lg:grid-cols-[1.3fr_1fr]">
            <Card className="rounded-md border border-line bg-paper shadow-lg">
              <CardHeader className="pb-2"><CardTitle className="text-base font-semibold text-ink">Choose & Order</CardTitle></CardHeader>
              <CardContent>
  const [presets, setPresets] = useState<Array<{ name: string; list: string[] }>>([])
              <div className="mt-4 flex items-center gap-3">
                <Checkbox checked={dryRun} onCheckedChange={(v) => setDryRun(v === true)} className="rounded-none" />
                <span className="text-base text-[hsl(var(--muted-foreground))]">Dry run</span>
                <Button size="sm" variant="outline" className="ml-auto rounded-none border-line text-ink hover:bg-[hsl(var(--muted))]" onClick={runPreview} disabled={selected.length === 0 || loading}>
                  <Eye className="mr-2 h-4 w-4" /> Preview
                </Button>
              </div>
      if (initialStack && initialStack.length) { setSelected(initialStack); seeded = true }
              <div className="mt-4 space-y-2">
                <div className="flex items-center gap-2">
                  <input value={presetName} onChange={(e) => setPresetName(e.target.value)} placeholder="Save current stack as…" className="h-10 w-80 rounded-none border border-line bg-paper px-3 text-ink placeholder:text-[hsl(var(--muted-foreground))]" />
        const url = new URL(window.location.href)
        const stackRaw = url.searchParams.get("stack") || ""
        if (stackRaw) {
          const list = decodeURIComponent(stackRaw).split(",").map((s) => s.trim()).filter(Boolean)
          if (list.length) setSelected(list)
        }
      } catch {}
    }
  ...
  useEffect(() => {
    if (!open) return
              <div className="mt-4 flex items-center gap-2">
                <input value={compareUserId} onChange={(e) => setCompareUserId(e.target.value)} placeholder="Compare with userId (optional)" className="h-10 w-96 rounded-none border border-line bg-paper px-3 text-ink placeholder:text-[hsl(var(--muted-foreground))]" />
      const arr = raw ? JSON.parse(raw) : []
              </CardContent>
            </Card>
  }, [open])
            <Card className="rounded-md border border-line bg-paper shadow-lg">
              <CardHeader className="pb-2"><CardTitle className="text-base font-semibold text-ink">Preview</CardTitle></CardHeader>
              <CardContent className="space-y-3">
    try {
                <div className="flex items-center gap-2 text-base text-[hsl(var(--muted-foreground))]"><Loader2 className="h-5 w-5 animate-spin" /> Loading…</div>
        method: "POST",
                <div className="text-sm text-[hsl(var(--muted-foreground))]">Run a preview to see effective values and sources.</div>
        body: JSON.stringify({ templateNames: selected, baseUserId }),
      })
      const payload = await res.json().catch(() => null)
      if (!res.ok) throw new Error(payload?.error || "Preview failed")
      setPreview({ result: payload.result || {}, sources: payload.sources || {}, conflicts: payload.conflicts || [] })

      // Load base user booleans to estimate changed fields (single user context)
      try {
        if (baseUserId) {
          const uRes = await fetch(`/api/users/${encodeURIComponent(baseUserId)}`, { cache: "no-store" })
          const u = await uRes.json().catch(() => null)
                  <div className="max-h-[60vh] overflow-auto rounded-md border border-line shadow-inner">
                    <table className="w-full text-base">
          NEWSLETTER_KEYS.forEach((k) => {
                        <tr className="border-b border-line text-left font-semibold">
                          <th className="px-4 py-2">Field</th>
                          <th className="px-4 py-2">Base</th>
                          {previewCompare && <th className="px-4 py-2">Compare</th>}
                          <th className="px-4 py-2">Source</th>
          setBaseBooleans({})
        }
      } catch { setBaseBooleans({}) }

      // Optional compare user side
      if (compareUserId && compareUserId.trim()) {
        const rc = await fetch("/api/templates/merge-preview", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ templateNames: selected, baseUserId: compareUserId.trim() }),
                              ...
                              className={`border-b border-line/50 ${differs ? 'bg-yellow-50 dark:bg-yellow-950/20' : ''}`}
        if (rc.ok && pc) setPreviewCompare({ result: pc.result || {}, sources: pc.sources || {} })
        else setPreviewCompare(null)
      } else {
        setPreviewCompare(null)
                              <td className="px-4 py-2 capitalize text-ink">{k.replace(/-/g, " ")}</td>
                              <td className="px-4 py-2">{vA === true ? "✓" : vA === false ? "✗" : String(vA)}</td>
      setError(e?.message || "Preview failed")
                                <td className="px-4 py-2">{vB === true ? "✓" : vB === false ? "✗" : String(vB ?? "—")}</td>
  }
                              <td className="px-4 py-2">
                                <motion.span key={`src-${k}-${src.join('+')}`} initial={{ opacity: 0 }} animate={{ opacity: 1, scale: [1, 1.04, 1] }} transition={{ duration: 0.25 }}>
    ...
    try {
      ...
      const payload = await res.json().catch(() => null)
      if (!res.ok) throw new Error(payload?.error || "Apply failed")
      const wrote = Array.isArray(payload?.results) ? payload.results.reduce((s: number, r: any) => s + (r?.wrote || 0), 0) : 0
      onApplied?.({ wrote, results: payload?.results || [] })
              </CardContent>
            </Card>
          </div>
          ...
          {/* Summary strip */}
          {preview && (
            <div className="flex items-center justify-between border-t border-line bg-paper/95 px-6 py-3 text-sm text-[hsl(var(--muted-foreground))]">
              <div>
                {Array.isArray(target?.ids) && target.ids.length > 0 && (
                  <span>{target.ids.length} {target.type === "group" ? "group(s)" : "user(s)"}</span>
                )}
                <span className="mx-2">•</span>
                <span>Preview updated • ready to {dryRun ? "dry run" : "apply"}</span>
              </div>
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" className="rounded-none border-line text-ink hover:bg-[hsl(var(--muted))]" onClick={copyShareLink} disabled={selected.length === 0}>Copy Link</Button>
                <Button variant="outline" size="sm" className="rounded-none border-line text-ink hover:bg-[hsl(var(--muted))]" onClick={() => onOpenChange(false)}>Close</Button>
  ...
  const savePreset = () => {
    ...
    const next = [...presets.filter((p) => p.name !== name), { name, list: selected }]
    setPresets(next)
    try { localStorage.setItem("template_stacks", JSON.stringify(next)) } catch {}
    setPresetName("")
    toast({ title: "Stack saved", description: `Saved preset “${name}”.` })
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="h-[92vh] w-[96vw] max-w-none rounded-none border border-line bg-background text-ink shadow-2xl">
          ...
          <div className="grid h-full grid-cols-1 gap-4 md:grid-cols-2">
            <Card className="rounded-md border border-line bg-paper shadow-lg">
              ...
              <div className="mt-3 flex items-center gap-3">
                <Checkbox checked={dryRun} onCheckedChange={(v) => setDryRun(v === true)} className="rounded-none" />
                <span className="text-base text-[hsl(var(--muted-foreground))]">Dry run</span>
                <Button size="sm" variant="outline" className="ml-auto rounded-none border-line text-ink shadow-sm hover:bg-[hsl(var(--muted))]" onClick={runPreview} disabled={selected.length === 0 || loading}>
                  <Eye className="mr-2 h-4 w-4" /> Preview
                </Button>
              </div>
              {/* Presets (local) */}
              <div className="mt-3 space-y-2">
                <div className="flex items-center gap-2">
                  <input value={presetName} onChange={(e) => setPresetName(e.target.value)} placeholder="Save current stack as…" className="h-10 w-72 rounded-sm border border-line bg-paper px-3 text-ink placeholder:text-[hsl(var(--muted-foreground))]" />
                  <Button size="sm" variant="outline" className="rounded-none border-line text-ink shadow-sm hover:bg-[hsl(var(--muted))]" onClick={savePreset} disabled={!presetName.trim() || selected.length === 0}>Save Stack</Button>
                  <Button size="sm" variant="outline" className="rounded-none border-line text-ink shadow-sm hover:bg-[hsl(var(--muted))]" onClick={() => {
                    try { navigator.clipboard.writeText(JSON.stringify(presets, null, 2)); toast({ title: "Stacks exported", description: "Copied presets JSON to clipboard." }) } catch {}
                  }}>Export</Button>
                  <Button size="sm" variant="outline" className="rounded-none border-line text-ink shadow-sm hover:bg-[hsl(var(--muted))]" onClick={async () => {
                    const raw = window.prompt("Paste presets JSON:")
                    if (!raw) return
                    try {
                      const arr = JSON.parse(raw)
                    ...
                    } catch (e) { toast({ title: "Import failed", description: "Invalid JSON.", variant: "destructive" as any }) }
                  }}>Import</Button>
                </div>
                {presets.length > 0 && (
                  <div className="flex flex-wrap items-center gap-2">
                    {presets.map((p) => (
                      <div key={p.name} className="flex items-center gap-1">
                        <Button size="sm" variant="outline" className="rounded-none border-line text-ink shadow-sm" onClick={() => loadPreset(p.name)} title={`Load ${p.name}`}>{p.name}</Button>
                        <Button size="icon" variant="ghost" className="h-6 w-6 rounded-none" onClick={() => deletePreset(p.name)} aria-label={`Delete ${p.name}`}>×</Button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {/* Compare user */}
              <div className="mt-3 flex items-center gap-2">
                <input value={compareUserId} onChange={(e) => setCompareUserId(e.target.value)} placeholder="Compare with userId (optional)" className="h-10 w-80 rounded-sm border border-line bg-paper px-3 text-ink placeholder:text-[hsl(var(--muted-foreground))]" />
              </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-md border border-line bg-paper shadow-lg">
              ...
              <CardContent className="space-y-2">
              {loading ? (
                <div className="flex items-center gap-3 text-[hsl(var(--muted-foreground))]"><Loader2 className="h-5 w-5 animate-spin" /> <span className="text-base">Loading…</span></div>
              ) : !preview ? (
                <div className="text-sm text-[hsl(var(--muted-foreground))]">Run a preview to see effective values and sources.</div>
              ) : (
                <div className="space-y-2">
                  ...
                  <div className="max-h-[56vh] overflow-auto rounded-none border border-line">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {Object.keys(preview.result).filter(k => NEWSLETTER_KEYS.includes(k as any)).slice(0, 200).map((k) => {
                          const vA = preview.result[k]
                          const vB = previewCompare?.result?.[k]
                          const src = preview.sources[k] || []
                          const differs = previewCompare ? vA !== vB : false
                          return (
                            <motion.tr
                              key={k}
                              className={`border-b border-line/50 ${differs ? 'bg-yellow-50 dark:bg-yellow-950/20' : ''}`}
                              initial={differs ? { backgroundColor: 'rgba(250,204,21,0.45)' } : false as any}
                              animate={differs ? { backgroundColor: 'rgba(250,204,21,0.12)' } : {} }
                              transition={{ duration: 0.6, ease: 'easeOut' }}
                            >
                              <td className="px-3 py-1 capitalize text-[hsl(var(--muted-foreground))]">{k.replace(/-/g, " ")}</td>
            ...
            <div>
              ...
              {(() => {
                try {
                  const keys = Object.keys(preview.result)
                  const boolKeys = keys.filter(k => typeof preview!.result[k] === 'boolean')
                  const changed = baseUserId && Object.keys(baseBooleans).length ? boolKeys.filter(k => baseBooleans[k] !== preview!.result[k]).length : boolKeys.length
                  return <span className="ml-2">• {changed} field{changed === 1 ? '' : 's'} {baseUserId ? 'will change (vs base)' : 'in stack'}</span>
                } catch { return null }
              })()}
            </div>
          </div>
        ...
        <div className="mt-2 flex justify-end gap-3 border-t border-line px-2 pt-3">
          <Button variant="outline" className="h-10 rounded-none border-line px-4 text-base text-ink hover:bg-[hsl(var(--muted))]" onClick={copyShareLink} disabled={selected.length === 0}>Copy Link</Button>
          <Button variant="outline" className="h-10 rounded-none border-line px-4 text-base text-ink hover:bg-[hsl(var(--muted))]" onClick={() => onOpenChange(false)}>Close</Button>
          <Button onClick={apply} disabled={selected.length === 0 || loading} className="h-10 rounded-none bg-ink px-5 text-base text-paper shadow hover:bg-ink/90">
            <Play className="mr-2 h-5 w-5" /> {dryRun ? "Run Dry Apply" : "Apply"}
          </Button>
        </div>
      </DialogContent>
